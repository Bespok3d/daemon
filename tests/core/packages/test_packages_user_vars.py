...
the suite reference these as packages.validate_user_vars / packages._expand / _load_user_vars."""
...
def test_symbols_reexported_from_package_namespace() -> None:
    ...
    assert packages._expand is user_vars._expand
    assert packages._persist_user_vars is user_vars._persist_user_vars
    assert packages._load_user_vars is user_vars._load_user_vars
    assert packages._with_plugin_venv is user_vars._with_plugin_venv
    assert packages._missing_required_vars is user_vars._missing_required_vars
...
def test_expand_replaces_longest_key_first() -> None:
    ...
    assert user_vars._expand("$BESPOK3D_KLIPPER/main.cfg", vars) == "/root/klipper/main.cfg"


def test_persist_and_load_round_trip(tmp_path: Path) -> None:
    user_vars._persist_user_vars(tmp_path, {"SERVER": "10.0.0.1"})
    assert user_vars._load_user_vars(tmp_path) == {"SERVER": "10.0.0.1"}


def test_persist_skips_empty_and_load_defaults_to_empty(tmp_path: Path) -> None:
    user_vars._persist_user_vars(tmp_path, {})
    assert not (tmp_path / "user_vars.json").exists()
    assert user_vars._load_user_vars(tmp_path) == {}


def test_with_plugin_venv_exposes_venv_path() -> None:
    full_vars = user_vars._with_plugin_venv({"BESPOK3D": "/userdata/bespok3d"}, "spoolman")
...
def test_missing_required_vars_lists_required_and_absent() -> None:
    ...
    assert user_vars._missing_required_vars(manifest, {"SERVER": "set"}) == ["TOKEN"]
